...
(function(){
  ...
  var PAT_HARD=[
    ...
    {w:2,p:[["r",2],["n",2]]},
    {w:1,p:[["r",4]]},
    {w:5,p:[["n",1],["n",1],["n",1],["n",1]]},
    {w:4,p:[["n",1],["n",1],["r",2]]},
    {w:3,p:[["n",1],["n",1],["n",1],["r",1]]},
    {w:2,p:[["n",1],["n",1],["r",1],["n",1]]},
    {w:2,p:[["r",1],["n",1],["n",1],["n",1]]},
    {w:2,p:[["n",1],["r",1],["n",1],["n",1]]},
    {w:1,p:[["n",1],["r",1],["r",1],["n",1]]}
  ];
  ...
  function sizeDrawCanvas(){if(!drawCanvas||!drawCtx){return;}DPR=window.devicePixelRatio||1;var cw=drawCanvas.clientWidth,ch=drawCanvas.clientHeight;if(!(cw>2&&ch>2)){var r=drawCanvas.getBoundingClientRect();cw=r.width;ch=r.height;}if(!(cw>2&&ch>2)){var g=document.getElementById('ob-guided');if(g){var gr=g.getBoundingClientRect();cw=gr.width;ch=gr.height;}}if(!(cw>2&&ch>2)){cw=window.innerWidth;ch=window.innerHeight;}var W=Math.max(1,Math.round(cw*DPR)),H=Math.max(1,Math.round(ch*DPR));if(drawCanvas.width!==W||drawCanvas.height!==H){drawCanvas.width=W;drawCanvas.height=H;}drawCtx.lineCap="round";drawCtx.lineJoin="round";drawCtx.lineWidth=3*DPR;}
  function clearDrawCanvas(){if(drawCtx&&drawCanvas){try{drawCtx.clearRect(0,0,drawCanvas.width,drawCanvas.height);}catch(e){}}}
  function applyDrawOn(){if(drawCanvas){drawCanvas.style.pointerEvents=drawOn?"auto":"none";}}
  function setDrawOn(v){drawOn=!!v;applyDrawOn();if(drawOn){sizeDrawCanvas();}}
  function evpos(ev){var r=drawCanvas.getBoundingClientRect();var cx=(ev.touches&&ev.touches[0])?ev.touches[0].clientX:ev.clientX;var cy=(ev.touches&&ev.touches[0])?ev.touches[0].clientY:ev.clientY;var sx=drawCanvas.width/(r.width||1);var sy=drawCanvas.height/(r.height||1);return [(cx-r.left)*sx,(cy-r.top)*sy];}
  ...
  function installDraw(){var orig=document.getElementById("draw-canvas");if(orig){orig.style.display="none";}var dbtn=null;var cands=document.querySelectorAll(".metronome--top button, .dock button, .topbar button, button");for(var i=0;i<cands.length;i++){if((cands[i].textContent||"").trim().toLowerCase()==="draw"){dbtn=cands[i];break;}}if(dbtn){dbtn.addEventListener("click",function(e){e.stopImmediatePropagation();e.preventDefault();setDrawOn(!drawOn);dbtn.classList.toggle("draw-on",drawOn);},true);}window.addEventListener("pointerup",function(){drawing=false;});window.addEventListener("resize",function(){if(drawCanvas){sizeDrawCanvas();}});}
  ...
  function okAt(o,d){if(o+d>16){return false;}if(d>=16){return o===0;}if(d>=8){return (o===0||o===8);}if(o<8&&o+d>8){return false;}return true;}
  ...
  function drawBeams(svg,groups){
    ...
    groups.forEach(function(gr){
      ...
      for(var j=0;j<gr.length;j++){
        ...
        var o=gr[j].o; var row=Math.floor(o/(L.rowCap*16)); var local=o-row*L.rowCap*16; var curBar=Math.floor(local/16);
        var leftBarline=L.LM+curBar*16*L.uSlot; var rightBarline=L.LM+(curBar+1)*16*L.uSlot;
        ...
        else { var wdt3=Math.min(0.4*slot,rightBarline-1-sxj); if(wdt3>1){mk(svg,"rect",{x:sxj,y:cy-31,width:wdt3,height:3,fill:"#222",rx:0.5});} }
        ...
        else if(!left){ var wdl2=Math.min(0.4*slot,sxj-leftBarline-1); if(wdl2>1){mk(svg,"rect",{x:sxj-wdl2,y:cy-31,width:wdl2,height:3,fill:"#222",rx:0.5});} }
      }
    });
  }
  ...
  function buildGuided(){var stage=document.querySelector(".stage");if(!stage){stage=document.body;}var old=document.getElementById("ob-guided");if(old){old.remove();}var w=document.createElement("div");w.id="ob-guided";w.innerHTML='<h1>Tap the rhythm</h1><svg id="ob-gsvg" viewBox="0 0 720 132" role="img" aria-label="rhythm"></svg><div class="ob-gbox" id="ob-gbox"></div>';stage.appendChild(w);gsvg=document.getElementById("ob-gsvg");gbox=document.getElementById("ob-gbox");drawCanvas=document.createElement("canvas");drawCanvas.id="ob-drawcanvas";drawCanvas.style.cssText="position:absolute;inset:0;z-index:50;pointer-events:none;touch-action:none;";w.appendChild(drawCanvas);drawCtx=drawCanvas.getContext("2d");draw(gsvg,gTiles,curAid());applyDrawOn();requestAnimationFrame(sizeDrawCanvas);clearDrawCanvas();aidBtns().forEach(function(b){b.addEventListener("click",function(){if(gsvg&&gTiles){draw(gsvg,gTiles,curAid());}});});gbox.addEventListener("pointerdown",function(e){if(!guidedActive){return;}if(gState==="tap"&&tapActive){e.preventDefault();var a=ac();registerTap(a?a.currentTime:0);return;}if(playing){return;}if(gState==="free"){e.preventDefault();startTap();}else if(gState==="done"){e.preventDefault();doNextFree();}});drawCanvas.addEventListener("pointerdown",function(ev){if(!drawOn){return;}ev.preventDefault();if(!drawCanvas.width||drawCanvas.width<2){sizeDrawCanvas();}drawing=true;var p=evpos(ev);lastDX=p[0];lastDY=p[1];});drawCanvas.addEventListener("pointermove",function(ev){if(!drawOn||!drawing){return;}ev.preventDefault();var p=evpos(ev);drawCtx.strokeStyle=drawColor;drawCtx.beginPath();drawCtx.moveTo(lastDX,lastDY);drawCtx.lineTo(p[0],p[1]);drawCtx.stroke();lastDX=p[0];lastDY=p[1];});}
