...
(function(){
  ...
  function layout(){var B=SLOTS/16;var rc=2;if(rc<1){rc=1;}L.rowCap=rc;L.bpRow=rc;L.numRows=Math.max(1,Math.ceil(B/rc));L.LM=56;L.RM=14;L.ROW_H=116;L.CY0=60;L.uSlot=(VW-L.LM-L.RM)/(rc*16);L.slot=L.uSlot;L.BW=16*L.uSlot;L.hr=6;}
  ...
  function setAid(mode){var ab=aidBtns(),i;for(i=0;i<ab.length;i++){var t=(ab[i].textContent||"").trim();var m=/none/i.test(t)?"none":(/ta|ti/i.test(t)?"syl":"num");if(m===mode){ab[i].classList.add("on");}else{ab[i].classList.remove("on");}}}
  function applyChoice(d){guidedDiff=d;try{window.__fpDiff=d;}catch(e){}applyBarOptions(d);try{if(d==="easy"){setBpm(80);setAid("syl");}else{setBpm(90);setAid("num");}}catch(e){}}
