(function(){
  ...
  (function(){
    ...
    window.addEventListener("message",function(ev){var d=ev.data;if(!d)return;
      ...
      else if(d.type==="praxis-settings"){if(d.settings){for(var k in d.settings){if(k in settings)settings[k]=d.settings[k];}if("lineCursor" in d.settings)settings.cursor=d.settings.lineCursor;}try{P().setMarkNotesDuringTapping(!!settings.marks);}catch(e){}if(!settings.cursor){cursor.stop();}else if(playing||flowState==="playing"||flowState==="tapping"){cursor.play(renderedRhythm,currentTempo);}renderAB();}
    });
