(function(){
  ...
  (function(){
    ...
    window.addEventListener('message',function(ev){var d=ev.data;if(!d)return;
      ...
      else if(d.type==='praxis-listen'){if(listenMode){listenMode=false;hardStop();flowState='done';post({type:'drill-state',playing:false,mode:'listen'});}else{hardStop();listenMode=true;if(!renderedRhythm||flowState==='init'){exerciseCount++;showExercise(randRhythm());}beginPlay();}}
