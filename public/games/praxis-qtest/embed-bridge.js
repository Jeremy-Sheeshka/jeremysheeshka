(function(){
  console.log("[bridge-build] playtap-v4");
  ...
  function makeCursor(getEnabled){
    ...
    function xForPos(row,pos){var p=row.pts,i;if(!p.length)return 0;if(p.length===1){var be=(Math.floor(p[0].pos/576)+1)*576;var fr=Math.min((pos-p[0].pos)/((be-p[0].pos)||1),1);return p[0].x+fr*300;}for(i=0;i<p.length-1;i++){if(pos>=p[i].pos&&pos<=p[i+1].pos){var span=(p[i+1].pos-p[i].pos)||1;var t=(pos-p[i].pos)/span;return p[i].x+(p[i+1].x-p[i].x)*t;}}if(pos<=p[0].pos)return p[0].x;var lastSpan=(p[p.length-1].pos-p[p.length-2].pos)||1;var extraT=(pos-p[p.length-1].pos)/lastSpan;return p[p.length-1].x+(p[p.length-1].x-p[p.length-2].x)*Math.min(extraT,3);}
    ...
    function setTempo(t){if(typeof t!=="number"||!raf)return;var om=(60000/(tempo||88))/144;var el=performance.now()-playStart;var c=el/om;tempo=t;var nm=(60000/(tempo||88))/144;playStart=performance.now()-c*nm;}return {play:play,stop:stop,isActive:isActive,setTempo:setTempo};
  }
  ...
  (function(){
    ...
    (function(){var p=P();if(!p)return;var origInit=p.initialisation;
      p.initialisation=function(cfg){
        ...
        if(rhythmImage&&!rhythmImage.__mkWrap){rhythmImage.__mkWrap=true;var _ri=rhythmImage,_a=_ri.showLabelOnComponent,_b=_ri.showLabelForUnknownTab;if(typeof _a==='function')_ri.showLabelOnComponent=function(){if(!settings.marks)return;return _a.apply(_ri,arguments);};if(typeof _b==='function')_ri.showLabelForUnknownTab=function(){if(!settings.marks)return;return _b.apply(_ri,arguments);};}useCfg.markNotesDuringTapping=!!settings.marks;
        useCfg.finishedTappingCallback=function(){completeExercise();};
        useCfg.afterStoppingCallback=function(){completeExercise();};
        var origStatus=(baseCfg&&baseCfg.changeStatusCallback)||(cfg&&cfg.changeStatusCallback)||null;
        useCfg.changeStatusCallback=function(s){
          if(s==="countin"){startBeatClock(currentTempo);}
          if(s==="play"){flowState="playing";if(settings.cursor)cursor.play(renderedRhythm,currentTempo);}
          if(s==="tap"){if(listenMode){listenMode=false;hardStop();flowState="done";setInfo("Click for next","","");post({type:"drill-state",playing:false,mode:"listen"});}else{flowState="playing";startClock();startBeatClock(currentTempo);if(settings.cursor)cursor.play(renderedRhythm,currentTempo);}}
        ...
        return origInit.call(this,useCfg);
      };
    ...
    window.addEventListener("message",function(ev){var d=ev.data;if(!d)return;
      ...
      else if(d.type==="praxis-listen"){if(listenMode&&playing){listenMode=false;hardStop();flowState="done";setInfo("Click for next","","");post({type:"drill-state",playing:false,mode:"listen"});}else{if(!renderedRhythm){exerciseCount++;showExercise(randRhythm());}beginListen();}}
      else if(d.type==="praxis-mute"){try{if(window.Howler)window.Howler.mute(!!d.muted);}catch(e){}}
      else if(d.type==="praxis-settings"){if(d.settings){for(var k in d.settings){if(k in settings)settings[k]=d.settings[k];}if("lineCursor" in d.settings)settings.cursor=d.settings.lineCursor;}try{P().setMarkNotesDuringTapping(!!settings.marks);if(!settings.marks&&rhythmImage&&rhythmImage.removeAllLabels)rhythmImage.removeAllLabels();}catch(e){}if(!settings.cursor){cursor.stop();}else if(playing||flowState==="playing"||flowState==="tapping"){cursor.play(renderedRhythm,currentTempo);}renderAB();}
    });
    ...
    window.addEventListener("load",function(){function __patchOpg(){if(window.MusicOpgave){try{window.MusicOpgave.userHasAnsweredTheRequiredNumberOfQuestionsCorrectly=function(){return false;};}catch(e){}try{window.MusicOpgave.forkertSvar=function(){};}catch(e){}}}function __firstShow(a){__patchOpg();if(rhythmImage&&P()){nextExercise();console.log(IS_COOP?"[coop-bridge] ready":"[fp-bridge] ready");post({type:"drill-ready",tempo:currentTempo});}else if((a||0)<40){setTimeout(function(){__firstShow((a||0)+1);},100);}else{nextExercise();post({type:"drill-ready",tempo:currentTempo});}}setTimeout(function(){__firstShow(0);},300);});
  })();
  return;
  }
  ...
  (function(){
    ...
    (function(){var p=P();if(!p)return;var origInit=p.initialisation;
      p.initialisation=function(cfg){
        ...
        useCfg.elementWithButton=null;useCfg.unitsPerStep=72;
        useCfg.includeCountIn=currentFlags.includeCountIn;useCfg.includePlaying=currentFlags.includePlaying;useCfg.includeTapping=currentFlags.includeTapping;useCfg.includeCountOut=currentFlags.includeCountOut;
        if(rhythmImage&&!rhythmImage.__mkWrap){rhythmImage.__mkWrap=true;var _ri=rhythmImage,_a=_ri.showLabelOnComponent,_b=_ri.showLabelForUnknownTab;if(typeof _a==='function')_ri.showLabelOnComponent=function(){if(!fsettings.marks)return;return _a.apply(_ri,arguments);};if(typeof _b==='function')_ri.showLabelForUnknownTab=function(){if(!fsettings.marks)return;return _b.apply(_ri,arguments);};}useCfg.markNotesDuringTapping=!!fsettings.marks;
        useCfg.finishedTappingCallback=function(){completeExercise();};
        useCfg.afterStoppingCallback=function(){completeExercise();};
        var origStatus=(baseCfg&&baseCfg.changeStatusCallback)||(cfg&&cfg.changeStatusCallback)||null;
        useCfg.changeStatusCallback=function(s){
          if(s==="countin"){startBeatClock(currentTempo);}
          if(s==="play"){flowState="playing";if(fsettings.cursor)cursor.play(renderedRhythm,currentTempo);}
          if(s==="tap"){if(listenMode){listenMode=false;hardStop();flowState="done";setInfo("Click for next","","");post({type:"drill-state",playing:false,mode:"listen"});}else{flowState="tapping";startBeatClock(currentTempo);if(fsettings.cursor)cursor.play(renderedRhythm,currentTempo);}}
          if(s==="stop"||s==="countout"){stopBeatClock();cursor.stop();if(flowState==="playing"||flowState==="tapping"){flowState="stopping";completeExercise();}}
          if(origStatus)origStatus(s);
        };
        return origInit.call(this,useCfg);
      };
    ...
    window.addEventListener("message",function(ev){var d=ev.data;if(!d)return;
      if(d.type==="praxis-metro"){if(d.tempo!=null)setTempo(d.tempo);if(d.playing){if(!renderedRhythm){exerciseCount++;showExercise(randRhythm());}beginPlay();}else{transportStop();}}
      ...
      else if(d.type==="praxis-settings"){if(d.settings){if("metroSound" in d.settings)fsettings.metroSound=d.settings.metroSound;if("master" in d.settings)fsettings.master=d.settings.master;if("marks" in d.settings)fsettings.marks=d.settings.marks;if("lineCursor" in d.settings){fsettings.lineCursor=d.settings.lineCursor;fsettings.cursor=d.settings.lineCursor;}}try{P().setMarkNotesDuringTapping(!!fsettings.marks);if(!fsettings.marks&&rhythmImage&&rhythmImage.removeAllLabels)rhythmImage.removeAllLabels();}catch(e){}if(!fsettings.cursor){cursor.stop();}else if(playing||flowState==="playing"||flowState==="tapping"){cursor.play(renderedRhythm,currentTempo);}}
    });
    (function(){var hidden=false;function tryHide(){if(hidden)return;var els=document.querySelectorAll("*"),i;for(i=0;i<els.length;i++){var el=els[i];if(el.children.length<=6&&/TEMPO/i.test(el.textContent)&&el.textContent.length<40){var pEl=el,j;for(j=0;j<4&&pEl.parentElement;j++)pEl=pEl.parentElement;pEl.style.setProperty("display","none","important");hidden=true;return;}}}try{var obs=new MutationObserver(function(){tryHide();});obs.observe(document.documentElement,{childList:true,subtree:true});}catch(e){}setTimeout(tryHide,1500);setTimeout(tryHide,4000);setTimeout(tryHide,9000);})();
    window.addEventListener("load",function(){function __patchOpg(){if(window.MusicOpgave){try{window.MusicOpgave.userHasAnsweredTheRequiredNumberOfQuestionsCorrectly=function(){return false;};}catch(e){}try{window.MusicOpgave.forkertSvar=function(){};}catch(e){}}}function __firstShow(a){__patchOpg();if(rhythmImage&&P()){nextExercise();console.log(IS_COOP?"[coop-bridge] ready":"[fp-bridge] ready");post({type:"drill-ready",tempo:currentTempo});}else if((a||0)<40){setTimeout(function(){__firstShow((a||0)+1);},100);}else{nextExercise();post({type:"drill-ready",tempo:currentTempo});}}setTimeout(function(){__firstShow(0);},300);});
  })();
