...
/* THE FIX: Use theme variables for the background and text */
.dropdown-answer { 
    max-height: 0; 
    overflow: hidden; 
    transition: all 0.3s ease-out; 
    padding: 0 1rem; 
    background: var(--theme-bg-card, #ffffff); /* Fallback to white, but follows theme */
}

/* Ensure text inside the answer follows the theme's text color */
.dropdown-answer p, .dropdown-answer strong, .dropdown-answer br {
    color: var(--theme-text) !important;
}

.dropdown-item.active .dropdown-answer { max-height: 10000px; padding: 1rem; }

/* 4. MAP & LEGEND STYLING */
...
<div class="legend">
<div class="legend-item"><div class="legend-color" style="width:14px; height:14px; border-radius:50%; background: #eab308;"></div><span class="legend-label">Behaviorism</span></div>
<div class="legend-item"><div class="legend-color" style="width:14px; height:14px; border-radius:50%; background: #22c55e;"></div><span class="legend-label">Cognitivism</span></div>
<div class="legend-item"><div class="legend-color" style="width:14px; height:14px; border-radius:50%; background: #3b82f6;"></div><span class="legend-label">Constructivism</span></div>
<div class="legend-item"><div class="legend-color" style="width:14px; height:14px; border-radius:50%; background: #f97316;"></div><span class="legend-label">Social Learning</span></div>
<div class="legend-item"><div class="legend-color" style="width:14px; height:14px; border-radius:50%; background: #ec4899;"></div><span class="legend-label">Neuroscience</span></div>
<div class="legend-item"><div class="legend-color" style="width:14px; height:14px; border-radius:50%; background: #64748b;"></div><span class="legend-label">Educational Tech</span></div>
<div class="legend-item"><div class="legend-color" style="width:14px; height:14px; border-radius:50%; background: #ef4444;"></div><span class="legend-label">Connectivism</span></div>
</div>
...
<script is:inline>
// ... (Your D3 GraphData and Logic stay the same to preserve the map)
const graphData = {
    nodes: [
...
const svgElement = d3.select("#concept-map");
const chartWidth = svgElement.node().parentElement.clientWidth;
const chartHeight = 800;
svgElement.attr("width", chartWidth).attr("height", chartHeight);
const chartG = svgElement.append("g");
...
const chartLink = chartG.append("g").selectAll("line").data(graphData.links).join("line").attr("class", d => d.class ? `link ${d.class}` : "link");

const chartNode = chartG.append("g").selectAll("g").data(graphData.nodes).join("g").attr("class", "node")
    .call(d3.drag().on("start", (event, d) => { if (!event.active) sim.alphaTarget(0.3).restart(); d.fx = d.x; d.fy = d.y; })
    .on("drag", (event, d) => { d.fx = event.x; d.fy = event.y; })
    .on("end", (event, d) => { if (!event.active) sim.alphaTarget(0); d.fx = null; d.fy = null; }));

chartNode.append("circle").attr("r", d => d.size).attr("fill", d => colorMap[d.group]);
chartNode.append("text").text(d => d.label).attr("dy", "0.35em")
    .style("font-size", d => d.size > 25 ? "18px" : "12px");

sim.on("tick", () => {
    chartLink.attr("x1", d => d.source.x).attr("y1", d => d.source.y).attr("x2", d => d.target.x).attr("y2", d => d.target.y);
    chartNode.attr("transform", d => `translate(${d.x},${d.y})`);
});

svgElement.call(d3.zoom().on("zoom", (e) => chartG.attr("transform", e.transform)));
</script>
