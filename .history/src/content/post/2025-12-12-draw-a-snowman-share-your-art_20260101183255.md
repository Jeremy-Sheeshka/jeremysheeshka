---
// src/components/SnowmanApp.astro
// This is the server-side frontmatter (runs at build time)
---

<div id="draw-a-snowman-app" class="not-prose">
  ...
  <div id="snowman-canvas-wrapper">
    <div id="snowman-canvas-inner">
      <img
        id="snowmanBase"
        src="/assets/images/blog/drawasnowman.png"
...
<script>
  ...
  submitSnowmanBtn?.addEventListener("click", async () => {
    ...
    if (snowmanBase) mctx.drawImage(snowmanBase, 0, 0, canvas.width, canvas.height);
    ...
    docs.forEach((snowmanDoc) => {
      ...
      if (moderatorMode) {
        ...
        delBtn.addEventListener("click", async () => {
          if (confirm("Delete this snowman?")) {
            try {
...
<style>
  ...
  /* Deep selector to apply styles to dynamic JS elements */
  :global(.snowman-card) {
    background: white;
  ...
  :global(.snowman-card img) {
    width: 100%;
    display: block;
  }
  :global(.moderator-delete-btn) {
    position: absolute;
    top: 6px;
    right: 6px;
    background: #dc2626;
