...
<style>
  /* 1. THE "SCORCHED EARTH" METADATA REMOVAL */
  /* Targets the navigation, footer, tags, icons, and that persistent read-time string */
  ...
  .dt-published, .post-date, .reading-time,
  .theme-toggle, #theme-toggle, [id*="theme-toggle"], [class*="theme-toggle"],
  .prose > div:first-of-type, aside, .sr-only,
  svg, .icon, [data-icon], a[href*="/tags/"],
  /* Targeting the specific flex containers that hold the meta text */
  .flex.items-center.gap-x-2, .flex.items-center.gap-x-1, .flex.items-center.gap-x-3,
  .mb-4.flex.flex-wrap.gap-2, .flex.items-center.text-sm,
  /* Target the first element after the title which is usually the meta row */
  .prose h1 + div, .prose h1 + p, article > div:first-child {
  ...
  .squiggle-title {
    ...
    font-size: 1.75rem;
    font-weight: bold;
    margin-top: 5rem; /* Massive margin to move past the hidden header area */
...
<script type="module" is:inline>
  // ... Firebase logic remains identical to preserve your drawing app functionality
